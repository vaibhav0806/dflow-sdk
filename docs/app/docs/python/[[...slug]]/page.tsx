...
export default async function Page(props: {
  ...
  const page = pythonSource.getPage(params.slug);
  if (!page) notFound();
...
export async function generateMetadata(props: {
  ...
  const description =
    ...
    `Learn about ${page.data.title} in the DFlow Python SDK documentation.`;
  const url = `${siteUrl}${page.url}`;

  return {
    title: `${title} | Python SDK`,
    description,
    openGraph: {
      title: `${title} | DFlow Python SDK`,
      description,
      url,
      type: 'article',
      siteName: 'DFlow SDK',
